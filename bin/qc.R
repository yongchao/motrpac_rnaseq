...
#read star qc info
star<-readqcinfo("post_align","star")
...
id2<-tolower(id)
colnames(qc53)<-c(paste0("%",id2),"median_5'_3'_bias",
                  paste0("%","umi_",id2),"umi_median_5'_3'_bias")

##collect data for rRNA, globin and phix and ERCC and duplicates when present
misc<-matrix(NA,NS,5)
colnames(misc)<-c("globin","rRNA","phix","ERCC","UMI_dup")
for(i in 1:NS){
    SID<-samples[i]
...
qc<-NULL
if(TRIM) qc<-cbind("reads_raw"=fastqc[,1],"%trimmed"=star[,1]/fastqc[,"Total Sequences"]*100,
                   "%trimmed_bases"=trim[,"percent_trimmed"])
qc<-cbind(qc,reads=star[,1],"%GC"=fastqc[,"%GC"],"%dup_sequence"=100-fastqc[,"total_deduplicated_percentage"],
          misc[,1:4])
if(UMI){
